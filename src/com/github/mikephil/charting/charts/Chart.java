...
public abstract class Chart<T extends ChartData<? extends DataSet<? extends Entry>>> extends
	...
	public DemoBase chartactivity;              //2015
	
	
	
    public DemoBase getChartactivity() {
		return chartactivity;
	}

	public void setChartactivity(DemoBase chartactivity) {
     ...
     * 
     */
    public void highlightTouch(Highlight high) throws Exception {
...
//            -------------»ñµÃxlabel-----------------------
            ...
            List list=mData.getXVals();   
            
            if(xIndex>=list.size())
            	return;
            String xString=(String) list.get(xIndex);
//            -------------------------------------
            ...
            if (xIndex <= mDeltaX && xIndex <= mDeltaX * mAnimator.getPhaseX()) {
                ...
                mMarkerView.refreshContent(e, dataSetIndex,xString);
